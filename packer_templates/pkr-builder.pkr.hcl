...
#    elevated_user     = "Administrator"
#    scripts = [
#      "${path.root}/scripts/windows/convert_disktogpt.ps1"
#    ]
#  }
  provisioner "file" {
    source = "${path.root}/win_answer_files/Wallpaper.jpg"
    destination = "c:\\users\\vagrant\\Wallpaper.jpg"
    except = var.is_windows ? null : local.source_names
  }
  provisioner "file" {
    source = "${path.root}/win_answer_files/GPO.zip"
    destination = "c:\\users\\vagrant\\GPO.zip"
    except = var.is_windows ? null : local.source_names
  }
  provisioner "powershell" {
    elevated_password = "vagrant"
    ...
    scripts = [
      ...
      "${path.root}/scripts/windows/nrec_specifics.ps1"
    ]
    except = var.is_windows ? null : local.source_names
  }
  provisioner "chef-solo" {
...
#    elevated_user     = "Administrator"
#    scripts = [
#      "${path.root}/scripts/windows/enable_administrator.ps1",
