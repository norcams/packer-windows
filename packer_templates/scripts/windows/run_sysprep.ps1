...
# to shutdown the computer, and the vagrant user cannot be deleted until
...
Set-NetFirewallRule -DisplayName 'WinRM HTTPS' -Enabled False -PassThru | Select -Property DisplayName, Profile, Enabled
Remove-NetFirewallRule -DisplayName "WinRM HTTPS"
Remove-NetFirewallRule -DisplayName "WINRM-HTTP-In-TCP-VAGRANT"
